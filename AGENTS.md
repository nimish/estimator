...
* For substantive tidal/met and shareable research reports: prioritize the Jupyter notebook (`examples/tidal_analysis.ipynb`) as the polished artifact; keep `examples/example_tidal_marimo.py` out of scope for agent-driven work unless the user explicitly requests Marimo changes (default to Jupyter, not Marimo polish). Prefer PACT-style structure: configuration-first cells, concise section headers, analysis-first plots, and factored plotting/helpers over tutorial-style exposition or long inline blocks; prefer human-readable station selection (e.g. `example_tidal.find_station` / `load_station` with `STATION_CATALOG`, or other catalog/name-based loaders) over raw numeric station IDs when it improves clarity.
...
* For shareable analysis notebooks and Marimo examples, keep tone and layout engineer-native and concise; avoid generic or visibly machine-generated structure. Present scalar fit metrics in formatted text or tables with appropriate units (e.g. percent vs absolute), not bar-chart summaries of single scalars. In Marimo notebooks, keep interactive cells/results near the top and move long helper/function blocks lower when possible.
...
* Example/run scripts use Click and Rich (deps in dependency-groups.examples); produce publication-quality figures (data overview, model summary, ablation) as PDF and PNG. Long grid/report scripts should log progress, support parallel workers, resume/skip completed work, auto-download cached inputs when needed, and handle Ctrl-C cleanly.
* Exogenous regressors must be standardized (zero-mean, unit-variance) before fitting; raw scale mismatch against [-1,1] Fourier features causes ill-conditioned design matrices and solver failures. In tidal example spline configs, positive lag indices mean the regressor leads the target (uses future samples); use only lags ≤ 0 for strictly causal fits. Exogenous interaction pairs are exact 2-way pairs over exog_config indices; duplicate, self, out-of-range, or bool indices are config errors, and referenced factors must include lag=0 so interactions are current-index only even when regressors also carry lags.
...
* TsgamMultiPeriodicConfig.periods, TsgamTrendConfig.grouping, and outlier period bucketing use the same regular-grid sample index as fit (not raw wall-clock hours); multiply intended hour-based widths by samples_per_hour for sub-hourly data. Trend types include explicit `NONLINEAR_INCREASING` and `NONLINEAR_DECREASING`; legacy `NONLINEAR` is a backwards-compatible alias for decreasing.
